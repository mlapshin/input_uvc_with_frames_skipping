...
static unsigned int frames_to_skip = 0;
...
void help(void) {
  ...
  fprintf(stderr, " [-f | --fps ]..........: frames per second\n" \
                  " [-s | --skip ].........: skip N frames before actual capture\n" \
...
void *cam_thread( void *arg ) {
  ...
  while( !pglobal->stop ) {
    int skip_this_frame = skipped_frames < frames_to_skip;

    /* grab a frame */
    if( uvcGrab(videoIn, skip_this_frame) < 0 ) {
